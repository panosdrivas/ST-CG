  ...
  count=0;
  count=count+A2(1,1:3)*p(1:3)*p(1);
  count=count+A2(2,1:4)*p(1:4)*p(2);
  
  for i=3:n-2
   count=count+A2(i,i-2:i+2)*p(i-2:i+2)*p(i);
  end
  
  count=count+A2(n-1,n-3:n)*p(n-3:n)*p(n-1);
  count=count+A2(n,n-2:n)*p(n-2:n)*p(n);
